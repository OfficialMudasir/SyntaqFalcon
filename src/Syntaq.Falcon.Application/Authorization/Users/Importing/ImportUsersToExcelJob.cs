...
namespace Syntaq.Falcon.Authorization.Users.Importing
{
    public class ImportUsersToExcelJob : AsyncBackgroundJob<ImportUsersFromExcelJobArgs>, ITransientDependency
    {
        ...
        private async Task CreateUserAsync(ImportUserDto input)
        {
            ...
            foreach (var displayRoleName in input.AssignedDisplayRoleNames)
            {
                // Check if roleName exists in roleList
                if (roleList.Any(r => r.DisplayName?.ToLowerInvariant() == displayRoleName?.ToLowerInvariant()))
                {
                    var role = await _roleManager.GetRoleByNameAsync(roleList.FirstOrDefault(r => r.DisplayName?.ToLowerInvariant() == displayRoleName?.ToLowerInvariant())?.Name);
