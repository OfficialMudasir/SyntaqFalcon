...
{
    [AbpAuthorize]
    public class ProfileAppService : FalconAppServiceBase, IProfileAppService
    {
        ...
        public async Task<CurrentUserProfileEditDto> GetCurrentUserProfileForEdit()
        {
            ...
            userProfileEditDto.QrCodeSetupImageUrl = user.GoogleAuthenticatorKey != null
                ? _googleTwoFactorAuthenticateService.GenerateSetupCode("Syntaq.Falcon",
        ...
        public async Task ChangePassword(ChangePasswordInput input)
        {
            ...
            if (await UserManager.CheckPasswordAsync(user, input.CurrentPassword))
            {
				...
				//	.ToList()
				//	.Any(hash => {
				//		var res = _passwordHasher.VerifyHashedPassword(user, hash.PasswordHash, input.NewPassword);
				...
				var hashedpassword = _passwordHasher.HashPassword(user, input.NewPassword);
        ...
        //STQ MODIFIED
        public async Task UpdateLogoPicture(UpdateProfilePictureInput input)
        {
            //byte[] byteArray;

            //var imageBytes = _tempFileCacheManager.GetFile(input.FileToken);
            var imageBytes = input.Image; // _cacheManager.GetCache("TempFileCacheName").Get(input.FileToken, ep => ep) as byte[];
            ...
            if (user.LogoPictureId.HasValue)
            {
                BinaryObject obj = _binaryObjectRepository.FirstOrDefault(user.LogoPictureId.Value);
                if (obj != null)
                {
                    _binaryObjectRepository.Delete(obj);
                }

            }

            var storedFile = new BinaryObject(AbpSession.TenantId, imageBytes);
            await _binaryObjectRepository.InsertAsync(storedFile);
            user.LogoPictureId = storedFile.Id;
