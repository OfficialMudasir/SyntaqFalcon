    ...
    public class HostSettingsAppService : SettingsAppServiceBase, IHostSettingsAppService
    {
        ...
        public async Task<HostSettingsEditDto> GetAllSettings()
        {
            return new HostSettingsEditDto
            {
                ...
                //STQ MODIFIED
                PaymentFormSettings = await GetPaymentFormSettingsAsync()
            };
        }


        private async Task<HostPaymentFormSettingsEditDto> GetPaymentFormSettingsAsync()
        {

            var setting = SettingManager.GetSettingValueForApplication<Guid>(AppSettings.HostManagement.PaymentFormId);
            var setting2 = SettingManager.GetSettingValueForApplication(AppSettings.HostManagement.StripeClientId);

            return new HostPaymentFormSettingsEditDto()
            {
                PaymentFormId = setting,
                StripeClientId = setting2
            };
        }
        ...
        private async Task<HostUserManagementSettingsEditDto> GetUserManagementAsync()
        {
            return new HostUserManagementSettingsEditDto
            {
                ...
                //#STQ Modified
                //10155_User_Acceptance_Tick_Issue
                IsUserAcceptanceRequired = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.IsUserAcceptanceRequired),
        ...
        public async Task UpdateAllSettings(HostSettingsEditDto input)
        {
            ...
            //STQ MODIFIED
            await UpdatePaymentFormSettingsAsync(input.PaymentFormSettings);
            //  await UpdateUserAcceptanceSettings(input.OtherSettings); 
        }

        public async Task<string> CheckIfAnyActiveUserAcceptanceTypes()
        {
            return await _userAcceptanceTypeRepository.FirstOrDefaultAsync(i => i.Active) != null ? "true" : "false";
        }

        private async Task UpdatePaymentFormSettingsAsync(HostPaymentFormSettingsEditDto input)
        {
            await SettingManager.ChangeSettingForApplicationAsync(AppSettings.HostManagement.PaymentFormId, Convert.ToString(input.PaymentFormId));
            await SettingManager.ChangeSettingForApplicationAsync(AppSettings.HostManagement.StripeClientId, Convert.ToString(input.StripeClientId));
        }
        ...
        private async Task UpdateUserManagementSettingsAsync(HostUserManagementSettingsEditDto settings)
        {
            ...
            await SettingManager.ChangeSettingForApplicationAsync(
                AppSettings.UserManagement.IsUserAcceptanceRequired,
                settings.IsUserAcceptanceRequired.ToString().ToLowerInvariant()
            );
        }
