...
namespace Syntaq.Falcon.Configuration.Tenants
{
    ...
    public class TenantSettingsAppService : SettingsAppServiceBase, ITenantSettingsAppService
    {
        ...
        //Refactor
        private async Task<TenantPaymentFormSettingsEditDto> GetPaymentFormSettingsAsync()
        {
            return new TenantPaymentFormSettingsEditDto()
            {
                PaymentFormId = await SettingManager.GetSettingValueAsync<Guid>(AppSettings.TenantManagement.PaymentFormId)
            };
        }
        ...
        // STQ MODIFIED
        private async Task UpdatePaymentFormSettingsAsync(TenantPaymentFormSettingsEditDto input)
        {
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.TenantManagement.PaymentFormId, Convert.ToString(input.PaymentFormId));
        }
        ...
        {
            if (_multiTenancyConfig.IsEnabled && !FalconConsts.AllowTenantsToChangeEmailSettings)
            {
                return;
            }
