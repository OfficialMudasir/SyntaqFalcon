...
namespace Syntaq.Falcon.DataExporting.Excel.EpPlus
{
    public abstract class EpPlusExcelExporterBase : FalconServiceBase, ITransientDependency
    {
        /// <summary>
        /// STQ ADDED
        /// </summary>
        public char[] BAD_CHARS = new char[] { '=', '(', ')', '!', '+', '-', '/', '*', '^', '%', '@' }; //simple example
        ...
        protected void AddObjects<T>(ExcelWorksheet sheet, int startRowIndex, IList<T> items, params Func<T, object>[] propertySelectors)
        {
            ...
            for (var i = 0; i < items.Count; i++)
            {
                for (var j = 0; j < propertySelectors.Length; j++)
                {
                    ...
                    {
                        var v = CleanDtoItem(propertySelectors[j](items[i]).ToString());
                        sheet.Cells[i + startRowIndex, j + 1].Value = v;
                    }
                    else
                    {
        ...
        protected string CleanDtoItem(string item)
        {
            // 1. Get a list of reserive Excel Characters and functions

            if (!string.IsNullOrWhiteSpace(item.Trim()))
            {
                //  remove exce lreserved characters //not allow excel start with @|=+-
                item = item.TrimStart('@', '|', '=', '+', '-', '\"');
                //replace in the middle +-=| to emplty 
                item = item.Replace("+", "");
                item = item.Replace("-", "");
                item = item.Replace("=", "");
                item = item.Replace("|", "");

                // add ' as prefex in the string to convet it into the text in excel
                //item = "\'" + item;
                return item;
            }
            else
