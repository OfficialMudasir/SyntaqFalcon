...
{
	[EnableCors("AllowAll")]
	public class FilesAppService : FalconAppServiceBase, IFilesAppService
	{
        ...
        public PagedResultDto<GetFileForViewDto> GetFiles(FilesDto filesDto)
		{

            ACLResultDto ACLResult = _ACLManager.CheckAccess(new ACLCheckDto()
            {
                Action = "Edit",
                EntityId = new Guid(filesDto.RecordMatterId),
                UserId = AbpSession.UserId,
                AccessToken = filesDto.AccessToken,
            ...
            if (ACLResult.IsAuthed)
            {
                ...
                string RecordMatterItemGroupId = filesDto.RecordMatterItemGroupId;
                if (!string.IsNullOrEmpty(filesDto.SubmissionId))
                {
                    var rmi = _recordMatterItemRepository.FirstOrDefault(e => e.SubmissionId == new Guid(filesDto.SubmissionId));
                    if (rmi != null)
                    {
                        var rm = _recordMatterRepository.FirstOrDefault(e => e.Id == rmi.RecordMatterId);
                        if (rm != null)
                        {
                            filesDto.RecordId = Convert.ToString(rm.RecordId);
                            filesDto.RecordMatterId = Convert.ToString(rmi.RecordMatterId);
                            RecordMatterItemGroupId = Convert.ToString(rmi.GroupId);
                        }
                    }
                }
                ...
                IEnumerable<IListBlobItem> blobs = container.ListBlobs("file-uploads" + "/" + filesDto.RecordId + "/" + filesDto.RecordMatterId + "/" + RecordMatterItemGroupId + "/", false);
                ...
                blobNames.ForEach(i =>
                {
                    ...
                    var Result = new GetFileForViewDto()
                    {

                        File = new FilesDto()
                        {
                            FileName = fname,
                            Size = (ToFileSize(Convert.ToDouble(blobLength))).ToString(),
                            RecordId = filesDto.RecordId,
                            RecordMatterId = filesDto.RecordMatterId,
                            RecordMatterItemGroupId = RecordMatterItemGroupId
                        }
                    };
                    FilesForView.Add(Result);
                });
                var totalCount = FilesForView.Count();
                return new PagedResultDto<GetFileForViewDto>(
                    totalCount,
                    FilesForView
                );
        ...
        public FileStreamResult DownloadFileById(FilesDto filesDto)
        {
            ...
            if (ACLResult.IsAuthed)
            {
                ...
                CloudBlockBlob blockBlob = container.GetBlockBlobReference("file-uploads" + "/" + filesDto.RecordId + "/" + filesDto.RecordMatterId + "/" + RecordMatterItemGroupId + "/" + filesDto.FileName);
                ...
                if (blockBlob.Exists())
                {
                    ...
                    var mimetype = GetMimeType(System.IO.Path.GetExtension(filesDto.FileName));
                    return new FileStreamResult(memStream, mimetype)
                    {
                        FileDownloadName = filesDto.FileName
                    };
                }
        ...
        public MemoryStream DownloadAllFilesByRecordMatterId(FilesDto filesDto)
		{
                ...
                using (ZipArchive zip = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
                {
                    blobNames.ForEach(i =>
                    {
                        ...
                        if (blockBlob.Exists())
                        {
                            MemoryStream memStream = new MemoryStream();
                            blockBlob.DownloadToStream(memStream);
                            string filename = i.Substring(86, i.Length - 86);
