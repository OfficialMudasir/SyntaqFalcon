...
namespace Syntaq.Falcon.MultiTenancy.HostDashboard
{
    ...
    public class HostDashboardAppService : FalconAppServiceBase, IHostDashboardAppService
    {
        ...
        private async Task<decimal> GetNewSubscriptionAmount(DateTime startDate, DateTime endDate)
        {
            ...
            var n = await _subscriptionPaymentRepository.GetAll()
                .Where(s => s.CreationTime >= startDate &&
                            s.CreationTime <= endDate &&
                            s.Status == SubscriptionPaymentStatus.Paid).ToListAsync();


            var n1 = n.Select(x => x.Amount).ToList();
            var n2 = n1.DefaultIfEmpty(0).Sum();
