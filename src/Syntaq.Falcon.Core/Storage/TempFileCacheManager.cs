...
namespace Syntaq.Falcon.Storage
{
    public class TempFileCacheManager : ITempFileCacheManager
    {
        ...
        {
            // STQ MODIFIED 
            // 1 day plus 1 hour time out for long forms and the file uplaod control
            _cacheManager.GetCache(TempFileCacheName).Set(token, content, new TimeSpan(1, 1, 0, 0)); // expire time is 1 min by default
        }
