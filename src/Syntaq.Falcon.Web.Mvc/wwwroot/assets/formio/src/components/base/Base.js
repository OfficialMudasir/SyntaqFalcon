...
var CKEDITOR = '/assets/ckeditor/ckeditor.js';
/**
...
export default class BaseComponent extends Component {
  ...
  build(state) {
    ...
    if (this.viewOnly) {
      ...
      if (!labelAtTheBottom) {
        //STQ Modified - 10910
        const costKey = this.root.data.requirementAndCostsItemKey && this.root.data.requirementAndCostsItemKey.key;
        let isFirstRowApplied = false;
        if (this.parent !== null) {
          // eslint-disable-next-line max-depth
          if (this.parent.parent !== null) {
            // eslint-disable-next-line max-depth
            if (this.parent.parent.component !== null) {
              isFirstRowApplied = this.parent.parent.component.applyLabelFirstRow;
            }
          }
        }
        if (isFirstRowApplied) {
          const row = this.row && this.row.split('-')[0];
          // eslint-disable-next-line max-depth
          if (row > 0) {
            //do not create label for Cost Line Item
          }
          else {
            this.createLabel(this.element);
          }
        }
        else if (costKey !== undefined) {
          const compKey = this.parent.parent ? this.parent.parent.key : this.root.component.key;
          // eslint-disable-next-line max-depth
          if (compKey === costKey) {
            // eslint-disable-next-line max-depth
            const row = this.row && this.row.split('-')[0];
            const costInnerComponents = this.parent.parent.component ? this.parent.parent.component.components[0].components : [];
            // eslint-disable-next-line max-depth
            if (row > 0) {
              //do not create label for Cost Line Item
              // eslint-disable-next-line max-depth
              if (costInnerComponents != null) {
                costInnerComponents.forEach((costComp) => {
                  // if logic is not applied and isFirstRowApplied is false ---> then show label
                  if (costComp.logic.length > 0 && !isFirstRowApplied) {
                    // if logic is applied and isFirstRowApplied is false ---> then hide label
                  }
                  else {
                    this.createLabel(this.element);
                  }
  ...
  get className() {
    ...
    // STQ Modified To Remove Extra Padding of required class
  ...
  //when using logic to update the place holder value
  ...
  addCKE(element, settings, onChange) {
    ...
    if (window._SyntaqBaseURI) {
      CKEDITOR = `${window._SyntaqBaseURI}/assets/ckeditor/ckeditor.js`;
    }
        ...
        var editor = ClassicEditor
          .create(element, {
            ...
            fontFamily: {
              options: [
                ...
                'Calibri, sans-serif', // add new fonts
          ...
          .then(editor => {
            ...
            editor.model.document.on('change', () => {
              ...
              this.editorValue = value; // Update the editorValue variable
              onChange(value);
            });
            return editor;
          })
          ...
          .catch(error => {
            ...
            console.warn('Build id: qqx873qhhuqa-gx71huglegef');
            console.error(error);
          });
        return editor;
  ...
  updateValue(flags, value) {
    ...
    try {
      if (changed) {
        this.rootValue.userInputChange = changed;
      }
    }
    catch (e) {
     //ignore
    }
    if (this.type === 'sfanumber') {
      changed = !_.isEqual(newValue, this.dataValue);
    }
  ...
  restoreValue() {
    if (this.hasSetValue) {
      ...
      //STQ MOdified
      if (this.component && this.component.editor === 'ckeditor') {
  ...
  setCustomValidity(message, dirty) {
    ...
    if (message) {
      ...
      if (this.options.highlightErrors) {
        this.removeClass(this.element, 'alert-danger');
      }
    ...
    for (let i = 0; i < this.inputs.length; i++) {
      const input = this.performInputMapping(this.inputs[i]);
      if (typeof input.setCustomValidity === 'function') {
        input.setCustomValidity(message, dirty);
      }
      try {
        //this.rootValue.inputs.push(input);
      }
      catch (e) {
        //ignore
      }
    ...
    //try {
    //  if (!this.rootValue.userInputChange) {
    //    if (this.rootValue.inputs.length > 0 && !this.rootValue.userInputChange) {
    //      for (let i = 0; i < this.rootValue.inputs.length; i++) {
    //        const input = this.performInputMapping(this.rootValue.inputs[i]);
    //        if (input.className.includes('is-invalid') && input.value.trim() === '') {
    //          firstMissingField = input;
    //          break; // Exit the loop when a missing field is found
    //        }
    //      }
  ...
  setLoading(element, loading) {
    ...
    if (element.loader) {
      if (loading) {
        ...
        element.loader.style.padding = '0px';
      }
      else {
