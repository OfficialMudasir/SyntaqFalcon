...
export default class NestedComponent extends BaseComponent {
  ...
  build(state, showLabel) {
    ...
    if (showLabel) {
      ...
      if (isFirstRowApplied) {
        ...
        //const costInnerComponents = this.parent.parent.component.components[0].components;
        // eslint-disable-next-line max-depth
        if (compKey === costKey) {
          // eslint-disable-next-line max-depth
          const row = this.row && this.row.split('-')[0];
          ...
          if (row > 0) {
            //do not create label for Cost Line Item
            /**
             * 1. if logic is not applied and isFirstRowApplied is false ---> then show label row > 0  ----- done
              2. if logic is applied and isFirstRowApplied is false ---> then hide label row > 0   ---------  done

              3. if logic is applied and isFirstRowApplied is true ---> then hide label row > 0

              4. if logic is not applied and isFirstRowApplied is true ---> then hide label row > 0
   ...
   */
  beforeNext() {
    return Promise.all(this.getComponents().map((comp) => {
      comp.data.TotalErrors = [];
      comp.data.userInputChange = false;
