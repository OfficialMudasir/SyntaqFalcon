(function ($) {
	app.modals.ManageACLModal = function () {
		...
		async function grantACL() {
			...
			await _aclService.grantACL(ACLForm).done(function () {
				abp.notify.info(app.localize('SharedSuccessfully'));
				_modalManager.close();
				abp.event.trigger('app.grantAccessSuccess');
			}).always(function () {
		...
		function revokeACL(ACLId) {
			...
			_aclService.revokeACL(ACL).done(function () {
				abp.notify.info(app.localize('RevokeSuccessfully'));
				_modalManager.close();
				abp.event.trigger('app.revokeAccessSuccess');
			}).always(function () {
				_modalManager.setBusy(false);
			});
		};
