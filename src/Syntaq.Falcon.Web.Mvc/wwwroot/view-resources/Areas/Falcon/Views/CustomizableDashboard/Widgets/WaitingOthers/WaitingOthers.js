$(function () {
	...
	var initDashboardContributor = function (data) {
		$('#WaitingOthersTableContainer').empty();
		$('#WaitingOthersTableContainer').append(`
                                <table id="WaitingOthersTableId"
		...
		$('#WaitingOthersTableId').DataTable({
			paging: true,
			data: data ? data : null,
			ajax: null,
			info: false,
			...
			columnDefs: [
				...
				},
				{
					...
					defaultContent: '',
					render: function (data, type, row) {
						var data = `<a class="OnClickLink" href="/Falcon/forms/load?OriginalId=${row.formId}&RecordMatterId=${row.recordMatterId}">${row.projectStepName}</a>`;
						return data;
					},
				},
				{
	...
	var getWaitingContributorData = function () {
		_tenantDashboardService.getDashboardProjectData({ tabType: 'C' })
			.done(function (result) {
				initDashboardContributor(result.projectsWaitOthers);
			});
	};

	getWaitingContributorData();

	$(".refreshWaitingOthersButton").click(function () {
		getWaitingContributorData();
	});
});