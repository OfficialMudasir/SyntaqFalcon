(function ($) {
	app.modals.ImportFormSchemaModal = function () {
		var _modalManager;
		...
		this.init = function (modalManager) {
			...
			$('#ImportSchemaForm input[name=file]').change(function () {
				$('#ImportSchemaForm').submit();
			});

			$('#ImportSchemaForm').ajaxForm({
				beforeSubmit: function (formData, jqForm, options) {

					var $fileInput = $('#ImportSchemaForm input[name=file]');
					...
					if (response.success) {
						abp.notify.success(app.localize('SchemaUpdated'));
						//_modalManager.close();
