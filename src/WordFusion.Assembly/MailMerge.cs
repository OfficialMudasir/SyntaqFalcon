    ...
    public class MailMerge {
        ...
        private void SetMergeField(MergeField mergeField, DataRow rowData)
        {
            ...
            try
            {
                    ...
                    if (mergeField.Name != null)
                    {
                        ...
                        if (rowData != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (fieldname.ToLower().Contains("if"))
                                {

                                    if (debug)
                                    {
                                        string mergedFieldName = "";
                                        string MergedValues = "";
                                        //string _fieldname = Regex.Replace(fieldname, @"[\(\)\[\]]", "");
                                        string _fieldname = Regex.Replace(fieldname, @"[\(\)\[\]]", "").Trim();
                                        _fieldname = _fieldname.Replace("Contains", "Like");
                                        _fieldname = _fieldname.Replace("*", "%");

                                        string[] parts = _fieldname.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                                        if (parts.Length > 0)
                                        {
                                            try
                                            {
                                                mergedFieldName = parts[1].Trim();
                                                MergedValues = Convert.ToString(rowData[mergedFieldName]);
                                            }
                                            catch (Exception ex) { }
                                        }

                                        ParseRegionValue(_fieldname, out string regionName, out string regionFilter);

                                        string debugfiltervalues = string.Empty;

                                        string filtercolumn = string.Empty;
                                        foreach (char c in _fieldname)
                                        {
                                        ...
                                        else
                                        {
                                            ...
                                            debugfiltervalues = " Values=(" + mergedFieldName.Trim() + "=" + MergedValues + ")";
                                        }
                                        if (_fieldname.ToLower().Contains("ifend:"))
                                        {
                        ...
                        if (mergeValue.ToLower().StartsWith("image/png")
                            ...
                            || mergeValue.ToLower().StartsWith("data:image/png")
                            || mergeValue.ToLower().StartsWith("data:image/jpeg")
                            || mergeValue.ToLower().StartsWith("data:image/jpg"))
                        {

                            // System.Drawing.Image img = new System.Drawing.Image();

                            // Handle cases where mergeValue starts with 'data:image/'
                            mergeValue = mergeValue.Replace("data:image/png;base64,", "");
                            mergeValue = mergeValue.Replace("data:image/jpeg;base64,", "");
                            mergeValue = mergeValue.Replace("data:image/jpg;base64,", "");

                            // Handle cases where mergeValue starts with 'image/'
                        ...
                        else if (!string.IsNullOrEmpty(mergeValue) && mergeValue.ToLower().Contains("<html>"))
                        {
                            this.SetHtmlMergeField(mergeField, mergeValue);
                        }
                ...
                {
                    this.ErrorList.Add(new MergeError(MergeErrorType.InvalidMergeField, "Setting Merge Field value error", "Something wrong with MergedFiled Merged"));
                }
        ...
        private void SetHtmlMergeField(MergeField mergeField, string mergeValue)
        {
            try
            {
                if (!string.IsNullOrEmpty(mergeValue) && mergeValue.ToLower().Contains("<html>"))
                {
                    ...
                    builder.InsertHtml(mergeValue);
                    Document htmlDoc = builder.Document;

                    InsertDocument(parentNode, htmlDoc);
                }
            }
            catch (Exception ex)
            {
